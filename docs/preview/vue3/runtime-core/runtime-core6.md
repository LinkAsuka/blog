---
title: runtime-core -> setup
createTime: 2025/01/21 20:28:29
tags:
  - VUE3源码
...
前面的组件确实可以写成是一个比较简单的 vue 例子，但是这和 vue2 的法简直是一样的。是的，前面的结构其实是 `optionsApi` 的写法，而 vue3 我们会使用 `setup` 函数来作为一个组件的入口。这样写可以提高代码的可复用性，增强逻辑的聚合度，让做相同事的函数方法能放到一起，不至于在写代码的时候上下横跳。

https://cn.vuejs.org/api/composition-api-setup

这里举一个简单的 setup 函数的例子：
